...
def write_stock_matrix(results: Sequence[ProfileMetrics], output_path: Path, corpus_rows: int) -> None:
    ...
    for item in results:
        ...
        free = STOCK_WIDTH * STOCK_HEIGHT - item.padded_area
        report.append(f"| {item.pixel_size}px / p{item.padding} | {item.retained_clean_glyph_count} | {item.generated_han_count} | {item.new_missing_han_count} | {item.glyph_count} | {item.bitmap_area:,} | {item.padded_area:,} | {free:+,} | {STOCK_WIDTH} | {item.packed_height} | {item.max_bbox_width}×{item.max_bbox_height} | {item.p50_bbox_width}×{item.p50_bbox_height} | {item.p95_bbox_width}×{item.p95_bbox_height} | {item.p99_bbox_width}×{item.p99_bbox_height} | {item.crop_count} | {item.overlap_count} | {'YES' if item.overflow else 'NO'} | YES | **{'PASS' if passed else 'FAIL'}** |")
    ...
    if chosen:
        report.append("Capacity status: `001C_STOCK_2048x1024_CAPACITY = SUFFICIENT`; no EXE patch is required for this stock-geometry fixture.")
    ...
    report.extend(["", "If 51/1 and (only if needed) 50/1 both fail, mark `001C_STOCK_2048x1024_CAPACITY = INSUFFICIENT_WITH_TESTED_ACCEPTABLE_PROFILES`, stop, and request a new size decision. Do not auto-generate 4096×4096.", ""])
    b.atomic_write_text(output_path, "\n".join(report))
...
def build_stock_fixture(base: b.CleanFont, entries: Sequence[b.CharsetEntry], font_path: Path, face_index: int, output_dir: Path, runtime_dir: Path, chosen: tuple[int, int], corpus_hash: str, corpus_rows: int) -> dict[str, object]:
    ...
    manifest = {
        ...
        "selector_corpus": f"{RESOURCE_CLASS}/{FILE_ID}", "selector_corpus_sha256": corpus_hash, "selector_corpus_rows": corpus_rows, "charset_unique_count": len(entries), "charset_han_count": sum(b.is_han(e.codepoint) for e in entries),
        ...
        "record_count": len(records), "mapped_count": len(parsed.font_data.mapped()), "required_codepoints_covered": required_ok, "preserved_clean_glyph_count": sum(1 for g in placed if g.glyph_source == "CLEAN_JPN_PRESERVED"), "generated_glyph_count": generated_count, "bitmap_area": sum(g.width * g.height for g in placed), "padded_area": sum((g.width + 2 * padding) * (g.height + 2 * padding) for g in placed), "atlas_usage_percent": round(100 * sum((g.width + 2 * padding) * (g.height + 2 * padding) for g in placed) / (STOCK_WIDTH * STOCK_HEIGHT), 4), "free_texels": STOCK_WIDTH * STOCK_HEIGHT - sum((g.width + 2 * padding) * (g.height + 2 * padding) for g in placed), "packed_width": STOCK_WIDTH, "packed_height": packed_height,
    ...
    report = [
        ...
        f"- Base: clean JPN `{SELECTOR}` only; no MLG/MLG_CN glyph source.", f"- Selector corpus: `{RESOURCE_CLASS}/{FILE_ID}`; {corpus_rows} production rows; {len(entries)} unique visible codepoints / {sum(b.is_han(e.codepoint) for e in entries)} Han.", f"- Source font: `{source_name}`; SHA256 `{source_hash}`.", f"- Raster profile: pixel size `{pixel_size}`, cell height `{cell_height}`, baseline `{baseline}`, padding `{padding}`.", f"- Atlas: `{STOCK_WIDTH}×{STOCK_HEIGHT}`, pitch `{STOCK_WIDTH}`, format `2`, tiled `0`, endian `0`, data_size `0x200000`.", f"- GlyphRecords / mapped: `{len(records)} / {len(parsed.font_data.mapped())}`; packed height `{packed_height}` / `{STOCK_HEIGHT}`.", f"- Plaintext XPR SHA256: `{b.sha256(plaintext)}`.", f"- Encrypted XPR SHA256: `{b.sha256(encrypted)}`.", "", "| static validation | result |", "|---|---|",
    ]
    report.extend(f"| {name} | {'PASS' if value else 'FAIL'} |" for name, value in checks.items())
    report.extend([f"- Bitmap area `{sum(g.width * g.height for g in placed):,}`; padded area `{sum((g.width + 2 * padding) * (g.height + 2 * padding) for g in placed):,}`; atlas usage `{100 * sum((g.width + 2 * padding) * (g.height + 2 * padding) for g in placed) / (STOCK_WIDTH * STOCK_HEIGHT):.3f}%`; free texels `{STOCK_WIDTH * STOCK_HEIGHT - sum((g.width + 2 * padding) * (g.height + 2 * padding) for g in placed):+,}`.", f"- Packed width/height `{STOCK_WIDTH}×{packed_height}` / stock height `{STOCK_HEIGHT}`.", "", "## RUNTIME NOT YET TESTED", "", "`CLEAN_JPN_001C_STOCK_SELF_OWNED_RUNTIME = NOT YET TESTED`", "", "`EXE_PATCH_REQUIRED = NO`", "", "Static success is not runtime proof. Replace only `001cbbd1.xpr` in a disposable test copy. Do not patch the EXE, do not replace 00c7, do not use an 001c runtime dimension patch or IFEO/debugger interception, and restore the backup after testing.", ""])
    (output_dir / "FONT_VALIDATION_REPORT.md").write_text("\n".join(report), encoding="utf-8")
    runtime_dir.mkdir(parents=True, exist_ok=True)
    (runtime_dir / SELECTOR).write_bytes(encrypted)
    (runtime_dir / "README_TEST.md").write_text("""# 001C stock-geometry Phase 2B.1 local runtime test
...
def main() -> int:
    ...
    if chosen is None:
        for pixel_size, padding in SALVAGE_PROFILES:
            glyphs, _name, _hash, _baseline, _height, _generated = rasterize_profile(base, entries, args.font_file, args.font_face_index, pixel_size)
            metrics = _simulate_pack(glyphs, pixel_size, padding)
            metrics = replace(metrics, retained_clean_glyph_count=sum(1 for g in glyphs if g.glyph_source == "CLEAN_JPN_PRESERVED"), generated_han_count=sum(1 for g in glyphs if g.is_han_override), new_missing_han_count=sum(1 for g in glyphs if g.is_han_override and g.codepoint not in base.parsed.font_data.mapped()))
            results.append(metrics)
            if metrics.crop_count == 0 and metrics.overlap_count == 0 and not metrics.overflow:
                chosen = (pixel_size, padding)
                break
    write_stock_matrix(results, args.output_dir / "FONT_001C_STOCK_ATLAS_MATRIX.md", len(rows))
    if chosen is None:
        print("001C_STOCK_2048x1024_CAPACITY=INSUFFICIENT_WITH_TESTED_ACCEPTABLE_PROFILES")
        print("No 4096x4096 output was generated.")
        return 2
    manifest = build_stock_fixture(base, entries, args.font_file, args.font_face_index, args.fixture_dir, args.runtime_test_dir, chosen, str(metadata["source_sha256"]), len(rows))
